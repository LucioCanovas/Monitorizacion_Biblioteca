...
def conexion(ip):
    try:
        ping_camara = ping3.ping(ip, timeout=10)
...
def crear_cola_mensajes(usuario, contraseña, ip_broker, cola): 
    ...
    while True:
        try:
            ...
            logging.error(f'Error al crear la cola rabbit: {e}')
            time.sleep(1)
...
ruta = f'{log_file_path}/log_{fecha_formateada}.log'

logging.basicConfig(filename=ruta, level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logging.getLogger('pika').setLevel(logging.CRITICAL) # evitar mensajes logging de rabbit, guardamos solo en las excepciones
